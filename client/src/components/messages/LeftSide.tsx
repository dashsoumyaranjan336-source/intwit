...
const LeftSide: React.FC = () => {
  ...
  // 🔴 JADU: Filter out blocked conversations BEFORE rendering
  const activeConversations = conversation.data?.filter((obj) => {
    // Har conversation me check karo ki dusra user (jo main nahi hu) blocked list me hai ya nahi
    const receiver = obj.recipients.find(r => r._id !== auth.user?._id);
    if (!receiver) return true; // Agar khud se chat hai (waisa hota nahi), toh dikhao
    
    // Agar receiver blocked list me hai, toh isko mat dikhao (return false)
    return !localBlocked.includes(receiver._id.toString());
  }) || [];

  const newMessage: IConversation[] = [];

  activeConversations.map((data: IConversation) => {
  ...
  return (
    <div className="messages-leftside-container">
      ...
      <div className="leftside-list-boxchat">
        {/* 🔴 MAP OVER THE FILTERED ARRAY (activeConversations) */}
        {activeConversations.length > 0 &&
          activeConversations.map((obj) => (
            <Link
              to={`/direct/${obj._id}`}
              key={obj._id}
