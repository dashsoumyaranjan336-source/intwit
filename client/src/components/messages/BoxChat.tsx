...
const BoxChat: React.FC<BoxChatProps> = ({ id }) => {
  ...
  // 🔴 SAFE & CLEAN REDUX CHECK (Bina kisi API Call / 401 error ke)
  const blockedList = (auth.user as any)?.blockedUsers || [];
  const receiverId = newArr[0]?.recipients?._id;
  
  const isBlocked = blockedList.some((b: any) => {
    const blockId = typeof b === 'object' ? b._id : b;
    return blockId?.toString() === receiverId?.toString();
  });

  const uploadImages = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (isBlocked) return; // Blocked hai toh image bhejna band
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (isBlocked) return; // Blocked hai toh message send mat hone do
    ...
    <>
      {newArr.length > 0 && (
        <div className="messages-rightside-container">
          ...
          {/* 🔴 SAFE HIDE LOGIC FOR BLOCKED USERS */}
          <div className="boxchat-footer">
            {isBlocked ? (
              <div style={{ width: "100%", padding: "20px", textAlign: "center", color: "#8e8e8e", borderTop: "1px solid #efefef", fontSize: "14px", fontWeight: "bold" }}>
